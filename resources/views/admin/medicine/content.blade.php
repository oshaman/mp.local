...
<div class="">
    <table class="table">
        ...
        @if(is_array($drugs) && !empty($drugs[0]))
            <tbody>
            @foreach ($drugs as $drug)
                <tr>
                    ...
                    <td>{{ $drug->title }}</td>
                    <td>
                        {!! Form::open(['url' => route('medicine_edit',['drug'=> $drug->id]),'class'=>'form-horizontal','method'=>'GET']) !!}
                        {!! Form::button('Редактировать', ['class' => 'btn btn-warning','type'=>'submit']) !!}
                        {!! Form::close() !!}
                    </td>
                    <td>
                        {!! Form::open(['url' => route('medicine_delete',['drug'=> $drug->id]),'class'=>'form-horizontal','method'=>'GET']) !!}
