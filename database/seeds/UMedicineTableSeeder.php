...
class UMedicineTableSeeder extends Seeder
{
    /**
...
//            if($k < 67) continue;
//            if($k > 66) continue;
//            if ('Микст-Аллергены Бытовые' !== $medicine->title) continue;
            ...
            $data = $this->getItems($medicine->url, $alias, $c, $medicine->title);
            ...
            if (!empty($data['fabricator'])) {
                $res = \Fresh\Medpravda\Fabricator::select('id')->where('utitle', $data['fabricator_name'])->first();
                if (!empty($res->id)) {
                    $fabricator = $res->id;
                } else {
                    $fabricator = 10000;
            ...
            if (!empty($data['farm'])) {
                $res = \Fresh\Medpravda\Pharmagroup::select('id')->where('utitle', $data['farm'])->first();
                if (!empty($res->id)) {
                    $pharm = $res->id;
                } else {
                    $pharm = 10000;
            ...
            try {
                $res = \Fresh\Medpravda\Umedicine::create([
                    'title' => $data['title'],
                    'alias' => $alias,
...
//            content Images
    ...
    public function getItems($url, $alias, $c, $title)
    {
        $data = $c->request(str_replace('https://tabletki.ua/', 'uk/', $url));
        $html = str_get_html($data['html']);
        if (empty($html)) return false;
        if (empty($html->find('div#main-container-content ol', 0))) {
            return false;
        }
        $t = $html->find('div#main-container-content ol', 0)->lastChild()->innertext;
...
//        MainImage
        ...
        $maindiv = $html->getElementById("ctl00_MAIN_ContentPlaceHolder_TranslatedPanel");

//        dd($maindiv->outertext);
...
//dd($res);
    ...
    public function contentImg($src, $path, $width)
    {
        ...
        $img->resize($width, null, function ($constraint) {
            $constraint->aspectRatio();
        })->save(public_path() . '/asset/images/medicine/content_ukr/' . $path, 100);;

        return '/asset/images/medicine/content_ukr/' . $path;
